...
| Path | What |
|---|---|
| `catalog/exploits.yaml` | The 36 detectors (single source of truth). Schema + how-to in `catalog/README.md`. Validated by `tools/validate_catalog.py`; README table generated by `tools/gen_catalog_table.py`. |
| `poc/` | Foundry project: one `Vulnerable<X>`+`Safe<X>`+test per detector. `cd poc && forge test` = **69 green** (native). |
| `ethernaut/` | Wargame harness. **40/40 solved 🏆**; `cd ethernaut && forge test` runs 39 (paris), Cashback runs under the prague profile. Both in CI. |
| `dvd/` | Damn Vulnerable DeFi v4 solutions — **18/18** (needs the external DVD repo; not in CI). |
...
> **Build note:** `cd ethernaut && forge test` builds and runs cleanly on a fresh checkout
> (Linux CI included). The split is by EVM version, not architecture: the default **paris**
> profile runs the 39 classic levels (incl. x86-pinned `solc` 0.5.x/0.6.x, which Foundry
> fetches automatically), and **Cashback** runs under `FOUNDRY_PROFILE=prague` (Cancun/Prague
> for transient storage + EIP-7702). Each profile's `skip` list in `foundry.toml` excludes the
> other's incompatible-pragma sources. The `poc/` suite is all `^0.8.20`. **Do NOT delete the
> old levels.** (Apple Silicon may still need Rosetta for the x86 `solc` 0.5/0.6 binaries.)
