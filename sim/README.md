...
| `test/BeanstalkGovFlashloan_2022_04.t.sol` | **WIP (skipped)** — Beanstalk $181M flashloan-governance drain (Aave + Curve 3pool/metapool + the Beanstalk diamond). Full reconstruction present but `vm.skip(true)`: the Bean3CRV Curve *factory* metapool (EIP-1167 proxy → Vyper impl) reverts on `approve` under fork; needs more ABI/state work. Kept as the marquee multi-protocol target. |
