...
| `test/BeanstalkGovFlashloan_2022_04.t.sol` | **Real incident replay** — Beanstalk $181M flashloan-governance drain (2022-04). Forks at block 14,595,905; pre-proposes a malicious BIP, then in one tx flash-loans ~$1B (Aave) into the Bean3CRV silo for an instant supermajority, `emergencyCommit`s to drain the protocol's LP, unwinds, and repays — netting ~$42M USDC. Drives Aave + Curve (3pool + factory metapool) + the live Beanstalk diamond. Ground-truth instance of catalog `beanstalk-governance-flashloan`. |
